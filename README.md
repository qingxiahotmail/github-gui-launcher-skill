...
为了方便用户快速上手和后续使用，项目提供了详细的启动手册：

- **[STARTUP_GUIDE.md](STARTUP_GUIDE.md)** - 完整的项目启动指南，包含环境搭建、功能说明和常见问题解决方案

### 手册内容

- **项目概述**：TrendRadar 的核心功能和使用场景
- **环境搭建**：首次部署和依赖安装步骤
- **一键启动**：如何使用 `start_trendradar_simple.bat` 快速启动项目
- **数据结构**：项目文件和数据存储结构说明
- **常见问题**：解决方案和最佳实践

## 🔧 一键启动

项目提供了简化版的一键启动程序，方便快速运行各项功能：

- **启动程序**：`start_trendradar_simple.bat`
- **功能菜单**：
  1. 运行数据抓取并生成报告
  2. 仅生成 HTML 报告
  3. 启动 HTTP 服务器
...
1. 双击运行 `start_trendradar_simple.bat`
2. 在弹出的命令窗口中选择对应功能的数字
3. 按 Enter 键执行操作
4. 操作完成后自动返回主菜单
...
- **start_trendradar_simple.bat**：一键启动程序，提供菜单式操作界面
- **STARTUP_GUIDE.md**：详细的启动手册，包含完整的使用指南
- **config/config.yaml**：主配置文件，控制项目的各项功能
- **config/frequency_words.txt**：关键词配置文件，用于热点新闻过滤

## 📚 相关链接

- **GitHub 仓库**：[sansan0/TrendRadar](https://github.com/sansan0/TrendRadar)
- **启动手册**：[STARTUP_GUIDE.md](STARTUP_GUIDE.md)
- **一键启动**：[start_trendradar_simple.bat](start_trendradar_simple.bat)
